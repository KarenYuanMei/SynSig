...
def make_avg_score_df(i):
	...
	positive_filename='nb_brain_synapse_positives.csv'

	positive_genes=pd.read_csv(positive_filename)
	positives=positive_genes['genes'].tolist()

	negative_filename='nb_brain_synapse_negatives.csv'
...
def plot_mean_ROC():
	...
	plt.plot(ns_fpr, ns_tpr, linestyle='--', color='k', label='No Skill')
	...
	#plt.title('Distinguishing Synapse Genes from Brain-Expressed Genes using Non-Brain Features \n 5-Fold Cross-Validation', fontweight = 'bold')
