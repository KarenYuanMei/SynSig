...
class PairOfGenes:
	def __init__(self,gene1,gene2):
		...
		'chr_no_source_feature', 'qPhos_site_number', 'Phosphosite_mouse_no', 'Phosphosite_hu_no', 'pFAM_domain_number', 'pFAM_domain', 'protein_mass', 'Ensembl_aa_length', 'Ensembl_isoform_no', 
		'trans_count', 'gc_content', 'trans_len', 'gene_length', 'exon_no', 'cds_length', 'bioplex_source_feature', 'biogrid_source_feature']
...
#find the genes in all of the features:
	...
	def combine_features(self,gene1_feature,gene2_feature,feature_name):
		...
		'spleen_hpa_isoform_exp', 'gtex_no_brain_exp', 'appendix_hpa_isoform_exp', 'heart muscle_hpa_isoform_exp', 'small intestine_hpa_isoform_exp', 'epididymis_hpa_isoform_exp', 'testis_hpa_isoform_exp',
		'liver_hpa_isoform_exp', 'esophagus_hpa_isoform_exp', 'urinary bladder_hpa_isoform_exp', 'skeletal muscle_hpa_isoform_exp', 'tonsil_hpa_isoform_exp', 'prostate_hpa_isoform_exp', 
		'parathyroid gland_hpa_isoform_exp','adipose tissue_hpa_isoform_exp', 'smooth muscle_hpa_isoform_exp', 'rectum_hpa_isoform_exp', 'bone marrow_hpa_isoform_exp', 'gtex_no_brain_exp', 
		'HIP_RNA', 'DFC_RNA', 'V1C_RNA', 'AMY_RNA', 'MD_RNA', 'STR_RNA', 'CBC_RNA', 'HIP_Proteomics', 'DFC_Proteomics', 'V1C_Proteomics', 'AMY_Proteomics', 'MD_Proteomics', 'CBC_Proteomics']
		subtraction_features=['Phosphosite_hu_no', 'Phosphosite_mouse_no', 'qPhos_site_number', 'Ensembl_isoform_no', 'Ensembl_aa_length', 'pFAM_domain_number', 'protein_mass', "trans_count", 'gc_content', 'trans_len', 'gene_length', 'exon_no', 'cds_length']
		jaccard_features=['pFAM_domain', 'mentha_source_feature', 'bioplex_source_feature', 'biogrid_source_feature']
...
def create_GO_score_dict():
	df=pd.read_csv('nb_GO_training_score_matrix_for_brain_features.csv', index_col=[0])
...
#find the genes in all of the features:
	...
	#plt.show()
	plt.savefig("rf_no_brain_feature_imp_%s.pdf"%number, bbox_inches='tight')
	plt.close()
	...
	plt.scatter(yfit, y_test, s=0.5)
	#plt.ylim((-0.5,10))
	#plt.xlim((-0.5,10))
	plt.xlabel('Predicted Semantic Similarity')
	plt.ylabel('True Semantic Similarity')
	plt.title('RF Performance: Predicted vs. True; Pearsonr=%s'%pearson_corr, fontweight='bold')
	#plt.show()
	#plt.savefig("rf_no_brain_feature_correlation_%s.pdf"%number, bbox_inches='tight')
	#plt.close()
	...
	#df.to_csv('/Users/karenmei/Documents/Synapse_Ontology/NetworkClass/Entry_Ontology/synapse_10/nonbrain_features_brain_genes_pipeline/random_forest/ypredict_ytest_%s.csv'%number)
...
def plot_density(df, number):
	g=sns.jointplot(x=df.ytest, y=df.ypredict, kind='kde')
	#annot_kws = {'prop':{'family':'monospace', 'weight':'bold', 'size':15}}
	#g = sns.jointplot("ytest", "ypredict", data=df, kind="hex", annot_kws=annot_kws)
	#sns.regplot(x="ytest", y="ypredict", data=df, ax=g.ax_joint, scatter=False, color='red')
	# calculate r and p-value here
	ytest=df['ytest'].tolist()
	ypredict=df['ypredict'].tolist()

	print (spearmanr(ytest, ypredict))
	pearson_corr=pearsonr(ytest, ypredict)[0]
	pearson_corr=np.round(pearson_corr,2)
	p_value=pearsonr(ytest, ypredict)[1]
	...
	#plt.show()
	plt.savefig("rf_nonbrain_feature_density_%s.pdf"%number, bbox_inches='tight')
	plt.close()
...
if __name__ == '__main__':
	...
	for i in range(5):
		...
		training_file='training_genes_%s.csv'%i
		test_file='test_genes_%s.csv'%i
